...
export default function Home({ latestEpisodes, allEpisodes }: HomeProps) {
  ...
  return (<div className={styles.homepage}>
    <section className={styles.lastestEpisodes}>
      ...
      <ul>
        {latestEpisodes.map((episode, index) => (<li key={episode.id}>
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const episodes = data.map(epi => {
    return {
      id: epi.id,
      title: epi.title,
      thumbnail: epi.thumbnail,
      members: epi.members,
      publishedAt: format(parseISO(epi.published_at), ' d MM yy', { locale: ptBR }),
      duration: Number(epi.file.duration),
      durationAsString: convertDurationToTimeString(Number(epi.file.duration)),
